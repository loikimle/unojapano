...
class User_Registration_Extras_Ajax {
	...
	public static function delete_account() {
		...
		if ( $delete_account_flag ) {
			...
			self::send_delete_account_email( $user->ID );
			self::send_delete_account_admin_email( $user->ID );
	 ...
	 */
	public static function send_delete_account_email( $user_id ) {
		...
		$to_replace   = array( '{{username}}', '{{email}}', '{{blog_info}}', '{{home_url}}', '{{admin_email}}' );
		$replace_with = array( $username, $email, get_bloginfo(), get_home_url(), get_option( 'admin_email' ) );
	 ...
	 * @param int $user_id ID of the user.
	 */
	public static function send_delete_account_admin_email( $user_id ) {
