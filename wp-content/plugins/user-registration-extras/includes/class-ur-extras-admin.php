	...
	class User_Registration_Extras_Admin {
		...
		public function user_registration_after_register_mail( $success_params, $valid_form_data, $form_id, $user_id ) {
			...
			if ( in_array( $form_id, $activated_form_list ) ) {
				$this->send_auto_generated_password_email( $user_id );
		 ...
		 * @param int $user_id ID of the user
		 */
		private function send_auto_generated_password_email( $user_id ) {
			...
			$to_replace   = array( '{{username}}', '{{email}}', '{{blog_info}}', '{{home_url}}', '{{auto_pass}}' );
			$replace_with = array( $username, $email, get_bloginfo(), get_home_url(), $user_pass );
