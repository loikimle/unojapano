...
(function ($) {
	var UR_Payment_Admin = {
		...
		init: function () {
			...
			$(document).on(
				...
				function (event, data) {
					...
					var mollie_conditional_settings =
						UR_Payment_Admin.save_conditional_logic_settings_mollie();
					...
					if (mollie_conditional_settings.length > 0) {
						data.data['ur_mollie_conditional_integration'] =
							mollie_conditional_settings;
					}
					...
					data.data.payment_field_present = UR_Payment_Admin.check_payment_field_exists()
				}
			);
		...
		save_conditional_logic_settings_mollie: function () {
			var mollie_conditional_settings = new Array();
			var conditional_logic_element_mollie = $("div[data-source='mollie']").closest(
				'.ur_conditional_logic_container'
			);

			var form_fields = $(conditional_logic_element_mollie).find(
				'.ur-conditional-wrapper'
			);

			var enable_conditional_logic_mollie = $(conditional_logic_element_mollie)
				.find('#ur_use_conditional_logic')
				.val();
			var enable_conditional_logic_mollie = $(conditional_logic_element_mollie)
				.find('#ur_use_conditional_logic')
			...
			var integration = {
				enable_conditional_logic: enable_conditional_logic_mollie,
				conditional_logic_data: conditional_logic_data
			};
			mollie_conditional_settings.push(integration);
			return mollie_conditional_settings;
		},
		...
		check_locked_payment_fields: function (data) {
			...
			$.each(data.data.form_setting_data, function (key, settings) {
				...
				if (settings.name === 'user_registration_enable_mollie') {
					mollie_enabled = settings.value;
				}
			});
			...
			if (
				...
				if (!stripe_enabled && !anet_enabled && !mollie_enabled) {
					data.data['ur_payment_disabled'] = [
						{
							validation_status: false,
		...
		check_payment_field_exists: function() {
			var fields_to_check = $('.ur-input-grids').find('.ur-input-type-single-item, .ur-input-type-multiple_choice, .ur-payment-enabled-slider , .ur-input-type-stripe-gateway, .ur-input-type-quantity-field, .ur-input-type-subscription_plan, .ur-input-type-coupon-field, .ur-input-type-total-field, .ur-input-type-stripe-gateway, .ur-input-type-authorize-net-gateway, .ur-input-type-mollie-gateway');
			return fields_to_check.length > 0;
		}
	};
