<div class="wrap user-registration-content-restriction">
	<header class="user-registration-header ur-border-bottom ur-d-flex ur-align-items-center">
		<a class="ur-text-muted ur-border-right ur-d-flex ur-mr-2 ur-pr-2" href="<?php echo esc_attr( empty( $_SERVER['HTTP_REFERER'] ) ? '#' : $_SERVER['HTTP_REFERER'] ); ?>">
		...
		<div class="user-registration-editable-title">
			<input class="user-registration-editable-title__input urcr-content-access-rule-title-input" type="text" value="<?php esc_html_e( 'Untitled', 'user-registration-content-restriction' ); ?>" placeholder="<?php esc_html_e( 'Access Rule Title Here...', 'user-registration-content-restriction' ); ?>"/>
		...
		<div class="user-registration-switch ur-ml-auto">
			...
			<label class="urcr-enable-access-rule-label" for="urcr-enable-access-rule"><?php esc_html_e( 'Enabled', 'user-registration-content-restriction' ); ?></label>
		</div>
	</header>
	<form class="urcr-content-access-rule-creator-form" method="post">
		...
		<nav class="nav-tab-wrapper urcr-nav-tab-wrapper ur-my-1">
			<a href="#" class="urcr-tab nav-tab nav-tab-active" data-tab-content-selector="#urcr-tab-content-conditions-and-logic"><?php esc_html_e( 'Conditions & Logics', 'user-registration-content-restriction' ); ?></a>
			<a href="#" class="urcr-tab nav-tab" data-tab-content-selector="#urcr-tab-content-target-contents"><?php esc_html_e( 'Target Contents', 'user-registration-content-restriction' ); ?></a>
			<a href="#" class="urcr-tab nav-tab" data-tab-content-selector="#urcr-action-tab-content"><?php esc_html_e( 'Action', 'user-registration-content-restriction' ); ?></a>
		</nav>
		...
		<p class="submit urcr-arc-buttons-container">
			...
			<?php if ( ! isset( $GLOBALS['urcr_hide_save_button'] ) || ( isset( $GLOBALS['urcr_hide_save_button'] ) && true !== $GLOBALS['urcr_hide_save_button'] ) ) : ?>
				<button class="button-primary urcr-save-rule" disabled='true'>
					<?php echo esc_html( apply_filters( 'urcr_create_rule_label', esc_html__( 'Create Rule', 'user-registration-content-restriction' ) ) ); ?>
				</button>
			<?php endif; ?>
			...
			<?php if ( ! isset( $GLOBALS['urcr_hide_save_draft_button'] ) || ( isset( $GLOBALS['urcr_hide_save_draft_button'] ) && true !== $GLOBALS['urcr_hide_save_draft_button'] ) ) : ?>
				<button class="button-primary urcr-save-rule-as-draft" disabled='true' style="background:#463736 !important;border:none;">
					<?php echo esc_html( apply_filters( 'urcr_save_rule_as_draft_label', esc_html__( 'Save as Draft', 'user-registration-content-restriction' ) ) ); ?>
				</button>
			<?php endif; ?>
		</p>
	</form>
