...
<?php if ( HMWP_Classes_Tools::getOption( 'test_frontend' ) && HMWP_Classes_Tools::getOption( 'hmwp_mode' ) <> 'default' ) {
    ...
    ?>
    <div class="col-sm-12 p-0 mx-0 mt-0 mb-3">
            ...
            <div class="text-center my-4">
                <div class="hmwp_confirm" style="display: inline-block; margin-right: 5px;">
                    <form class="hmwp_frontendcheck_form" method="POST">
                        ...
                        <button type="button" class="btn rounded-0 btn-default btn-lg px-4 frontend_test"><?php echo esc_html__( 'Frontend Test', 'hide-my-wp' ); ?></button>
                    </form>
                </div>
                <div class="text-center" style="display: inline-block; margin-right: 5px;">
                    <button type="button" class="btn rounded-0 btn-default btn-lg px-4 login_test hmwp_modal" data-remote="<?php echo esc_url( site_url() . '/' . HMWP_Classes_Tools::getOption( 'hmwp_login_url' ) . '?nordt=1&hmwp_preview=' . HMWP_Classes_Tools::getOption( 'hmwp_disable_name' ) ) ?>" data-target="#frontend_test_modal"><?php echo esc_html__( 'Login Test', 'hide-my-wp' ); ?></button>
                </div>
            </div>
            ...
            <ol>
                <li>
                    ...
                    echo wp_kses_post(
                            ...
                            /* translators: 1: Opening <strong> tag. 2: Closing </strong> tag. */
                                    __( 'Run %1$sFrontend Test%2$s to check if the new paths are working.', 'hide-my-wp' ),
                                    '<strong>',
                                    '</strong>'
                            )
                    );
                ...
                <li>
                    ...
                    echo wp_kses_post(
                            sprintf(
                            /* translators: 1: Opening <strong> tag. 2: Closing </strong> tag. */
                                    __( 'Run %1$sLogin Test%2$s and log in inside the popup.', 'hide-my-wp' ),
                                    '<strong>',
                                    '</strong>'
                            )
                    );
                ...
                <li>
                    ...
                    echo wp_kses_post(
                            ...
                            /* translators: 1: Plugin name. 2: Opening anchor tag. 3: Closing anchor tag. */
                                    __( 'If you can\'t configure %1$s, switch to Deactivated Mode and %2$scontact us%3$s.', 'hide-my-wp' ),
            ...
            if ( defined( 'HMWP_DEFAULT_LOGIN' ) && HMWP_DEFAULT_LOGIN ) {
                ...
                <div class="wp-admin_warning col-sm-12 my-4 text-danger p-0 text-center">
                    <div class="mb-3">
                        ...
                        echo wp_kses_post(
                                sprintf(
                                /* translators: %s: Login URL rendered as a clickable link. */
                                        __( 'Your login URL is: %s', 'hide-my-wp' ),
                                        '<br /><a href="' . esc_url( $custom_login ) . '" target="_blank" rel="noopener">' . esc_url( $custom_login ) . '</a>'
                                )
                        );
                        ?>
                    </div>
            ...
            <?php } else { ?>
                <div class="wp-admin_warning col-sm-12 my-4 text-danger p-0 text-center">
                    <div class="mb-3">
                        ...
                        echo wp_kses_post(
                                sprintf(
                                /* translators: 1: Login URL rendered as a clickable link. 2: Safe URL rendered as a clickable link. */
                                        __( 'Your login URL will be: %1$s In case you can\'t log in, use the safe URL: %2$s', 'hide-my-wp' ),
                                        '<br /><a href="' . esc_url( $login_url ) . '" target="_blank" rel="noopener">' . esc_url( $login_url ) . '</a><br /><br />',
                                        '<br /><a href="' . esc_url( $safe_url ) . '" target="_blank" rel="noopener">' . esc_url( $safe_url ) . '</a>'
                                )
                        );
                        ?>
                    </div>
