				...
				<div class="user-registration-profile-fields">
					...
					<div class="user-registration-profile-header">
						<div class="user-registration-img-container" style="width:100%">
                            ...
                            $max_upload_size_format = size_format( $max_upload_size );

							?>
							<p class="user-registration-tips"><?php echo __( 'Max size: ', 'user-registration' ) . $max_upload_size_format; ?></p>
						</div>
						<header>
						...
						if ( has_action( 'uraf_profile_picture_buttons' ) ) {
							...
							<div class="uraf-profile-picture-upload">
								<p class="form-row " id="profile_pic_url_field" data-priority="">
									...
									<input type="file" id="ur-profile-pic" name="profile-pic" class="profile-pic-upload" accept="image/x-png,image/gif,image/jpeg" style="<?php echo ( $gravatar_image !== $image ) ? 'display:none;' : ''; ?>" />
						...
						<input type="file" id="ur-profile-pic" name="profile-pic" class="profile-pic-upload" accept="image/x-png,image/gif,image/jpeg" style="display:none" />
								<?php
							} else {
								?>
							<input type="file" id="ur-profile-pic" name="profile-pic" class="profile-pic-upload" accept="image/x-png,image/gif,image/jpeg" style="<?php echo ( $gravatar_image !== $image ) ? 'display:none;' : ''; ?>" />
								...
								foreach ( $data as $grid_key => $grid_data ) {
									...
									foreach ( $grid_data as $grid_data_key => $single_item ) {
										...
										if ( isset( $profile[ $key ] ) ) {
                                            ...
                                            $removeFieldsInProfileDetailPage = ['html', 'mailchimp'];
                                            if (in_array($single_item->field_key, $removeFieldsInProfileDetailPage)) {
                                                continue;
                                            }
